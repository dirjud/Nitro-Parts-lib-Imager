...
def rgb_plot_img(x, B=10):
    xd =(x>>(B-8)).astype(numpy.uint8)
    pylab.imshow(xd, interpolation='nearest')


def get_input_and_output_imgs(dev, num_rowsi, num_colsi, num_rowso=None, num_colso=None, depthi=None, deptho=None):
    if depthi is None:
        x = numpy.zeros([num_rowsi, num_colsi], dtype=numpy.uint16)
    else:
        x = numpy.zeros([num_rowsi, num_colsi, depthi], dtype=numpy.uint16)
    ...
    if deptho is None:
        ...
        y = numpy.zeros([num_rowso, num_colso, deptho], dtype=numpy.uint16)
